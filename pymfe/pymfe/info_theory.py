...
class MFEInfoTheory:
    ...
    def precompute_class_freq(
        ...
        if y is not None and "class_freqs" not in kwargs:
            _, class_freqs = _backend.unique(y, return_counts=True)
    ...
    def _calc_entropy(
        ...
        if value_freqs is None:
            _, value_freqs = _backend.unique(values, return_counts=True)

        return float(scipy.stats.entropy(np.asarray(_backend.to_cpu(value_freqs)), base=2))
    ...
    def ft_attr_ent(
        ...
        if _backend.get_array_module(C) is np:
            return np.apply_along_axis(func1d=cls._calc_entropy, axis=0, arr=C)

        return np.asarray([cls._calc_entropy(col) for col in _backend.to_cpu(C).T])
    ...
    def ft_class_conc(cls, C: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        return np.apply_along_axis(
            func1d=cls._calc_conc,
            axis=0,
            arr=_backend.to_cpu(C),
            vec_y=_backend.to_cpu(y),
        )
    ...
    def ft_mut_inf(
        ...
        return np.asarray(attr_ent) + class_ent - np.asarray(joint_ent)
