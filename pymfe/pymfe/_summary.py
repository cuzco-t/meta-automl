...
def _remove_nan(values: TypeValList) -> TypeValList:
    ...
    values = np.asarray(_backend.to_cpu(values), dtype=float)
...
def sum_histogram(
    ...
    try:
        freqs, _ = np.histogram(_backend.to_cpu(values), bins=bins)
...
def sum_skewness(
    ...
    skew_val = scipy.stats.skew(_backend.to_cpu(values), bias=bias)
...
def sum_kurtosis(
    ...
    kurt_val = scipy.stats.kurtosis(_backend.to_cpu(values), bias=bias)
...
def sum_nanstd(values: TypeValList, ddof: int = 1) -> float:
    ...
    return float(np.nanstd(_backend.to_cpu(values), ddof=ddof))
...
def sum_std(values: TypeValList, ddof: int = 1) -> float:
    ...
    return float(np.std(_backend.to_cpu(values), ddof=ddof))
...
def sum_nanvar(values: TypeValList, ddof: int = 1) -> float:
    ...
    return float(np.nanvar(_backend.to_cpu(values), ddof=ddof))
...
def sum_var(values: TypeValList, ddof: int = 1) -> float:
    ...
    return float(np.var(_backend.to_cpu(values), ddof=ddof))


def sum_nancount(values: TypeValList) -> int:
    """Count how many non-nan element in ``values``."""
    values = _backend.to_cpu(values)
...
def sum_naniq_range(values: TypeValList) -> float:
    ...
    return scipy.stats.iqr(_backend.to_cpu(values), nan_policy="omit")
