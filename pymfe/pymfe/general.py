...
class MFEGeneral:
    ...
    def ft_nr_bin(cls, X: np.ndarray) -> int:
        ...
        if xp is np:
            ...
            return int(np.sum(bin_cols))

        bin_cols = xp.array([xp.unique(col).size == 2 for col in X.T], dtype=bool)

        return int(_backend.to_cpu(xp.sum(bin_cols)))
