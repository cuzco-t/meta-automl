...
class MFEStatistical:
    ...
    def precompute_statistical_class(
        ...
        if y is not None and not {"classes", "class_freqs"}.issubset(kwargs):
            classes, class_freqs = _backend.unique(y, return_counts=True)
    ...
    def precompute_statistical_cor_cov(
        ...
        if N is not None and N.size:
            if "cov_mat" not in kwargs:
                precomp_vals["cov_mat"] = _backend.cov(N, rowvar=False, ddof=ddof)

            if "abs_corr_mat" not in kwargs:
                abs_corr_mat = np.abs(_backend.corrcoef(N, rowvar=False))
    ...
    def _can_cor_to_eigval(can_cors: np.ndarray) -> np.ndarray:
        ...
        """
        can_cors = np.asarray(_backend.to_cpu(can_cors), dtype=float)
    ...
    def _calc_can_cors(
        ...
        y_bin = ohe.fit_transform(np.asarray(_backend.to_cpu(y)).reshape(-1, 1))
